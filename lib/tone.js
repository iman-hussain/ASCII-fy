 ...
 */
export function adaptiveTone(colorDepth, stats, fileExt) {
...
function depthTier(depth) {
  if (depth <= 4)  return { contrastCeiling: 1.2, contrastDefault: 1.6,  brightnessCeiling: 0.10, brightnessMult: 1.5, brightnessDefault: 0.06, gammaCeiling: 0.5, gammaDefault: 1.2, saturationDefault: 1.5 };
  if (depth <= 16) return { contrastCeiling: 0.8, contrastDefault: 1.35, brightnessCeiling: 0.08, brightnessMult: 1.2, brightnessDefault: 0.04, gammaCeiling: 0.3, gammaDefault: 1.15, saturationDefault: 1.3 };
  if (depth <= 32) return { contrastCeiling: 0.5, contrastDefault: 1.2,  brightnessCeiling: 0.06, brightnessMult: 1.0, brightnessDefault: 0.03, gammaCeiling: 0.2, gammaDefault: 1.1, saturationDefault: 1.2 };
  if (depth <= 64) return { contrastCeiling: 0.3, contrastDefault: 1.15, brightnessCeiling: 0.04, brightnessMult: 0.8, brightnessDefault: 0.02, gammaCeiling: 0.15, gammaDefault: 1.05, saturationDefault: 1.1 };
  // 256+ / truecolor – minimal adjustment
  return { contrastCeiling: 0.15, contrastDefault: 1.05, brightnessCeiling: 0.03, brightnessMult: 0.5, brightnessDefault: 0.01, gammaCeiling: 0.1, gammaDefault: 1.0, saturationDefault: 1.0 };
}
 ...
 */
export async function sampleVideoLuminance(inputPath, outputWidth, meta) {
  const { spawn } = await import('node:child_process');
  const ffmpegPath = (await import('ffmpeg-static')).default;

  const scaledH = Math.max(1, Math.round((outputWidth / meta.width) * meta.height / 2));
  const args = [
    '-ss', '0',
    '-i', inputPath,
    '-frames:v', '1',
    ...
    '-vcodec', 'rawvideo',
    '-pix_fmt', 'rgb24',
    '-vf', `scale=${outputWidth}:${scaledH}:flags=fast_bilinear`,
    '-',
  ];
