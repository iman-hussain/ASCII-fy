...
async function initFFmpeg() {
	...
	if (typeof window !== 'undefined' && window.crossOriginIsolated) {
		console.log('[ASCII-fy] Cross-origin isolation is active (good!)');
	} else if (typeof self !== 'undefined' && self.crossOriginIsolated) {
		console.log('[ASCII-fy] Cross-origin isolation is active in worker (good!)');
	} else {
		console.warn('[ASCII-fy] Cross-origin isolation status unknown, but SharedArrayBuffer is available');
	}
	...
	console.log('[ASCII-fy] Downloading ~30MB of WebAssembly files...');
	...
	try {
		...
		try {
			...
			throw new Error('Failed to load FFmpeg from both CDNs. Please check your internet connection and try again.');
		}
	}
	return ffmpeg;
}
