...
// Dynamic import for gifenc to work in workers with absolute CDN URL
let gifenc = null;

async function loadGifenc() {
	if (gifenc) return;
	try {
		const gifencModule = await import('https://unpkg.com/gifenc@2.0.5/dist/gifenc.js');
		gifenc = gifencModule.default;
	} catch (err) {
