...
// Dynamic import for gifenc (local npm package)
let gifenc = null;
...
async function loadGifenc() {
	...
	try {
		...
		gifenc = gifencModule.default || gifencModule;
	} catch (err) {
		console.error('[ASCII-fy] Failed to load gifenc:', err);
		throw new Error('Failed to load gifenc: ' + err.message);
	}
}
 ...
 */
export async function createAsciiGifWriter({ width, height, fps, render, outputPath }) {
	await initGifenc();
	const encoder = GIFEncoder();
	const delay = Math.max(20, Math.round(1000 / (fps || 12)));
	const framesParams = [];

	// Force transparent background for GIF previews — only ASCII glyphs are drawn.
	// The GUI preview tab has its own background colour picker for viewing.
	const gifRender = { ...render, theme: { ...render?.theme, bg: 'transparent' } };
	...
	const finalize = async () => {
		...
		const renderedFrames = framesParams.map(f => renderFrameToRgba(f, width, height, gifRender));
		...
		const hasTransparentBg = gifRender?.theme?.bg === 'transparent';
