...
export function createAsciiGifWriter({ width, height, fps, render, outputPath }) {
	...
	const finalize = async () => {
		...
		for (let i = 0; i < renderedFrames.length; i++) {
			...
			// Apply base palette to get dense colour output BEFORE injecting transparency
			const index = applyPalette(pixels, basePalette);

			if (prevIndexBuffer) {
				for (let j = 0; j < index.length; j++) {
					// Inter-frame LZW deduplication - overwrite invariant pixels with transparency
			...
			encoder.writeFrame(index, imgW, imgH, {
				...
				delay,
				dispose: 1 // Do not dispose, layer deltas over the previous frame
			});
		}
