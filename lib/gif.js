...
function renderFrameToRgba(frame, width, height, render) {
	...
	// Fill background (supports transparent)
	const bgSetting = render?.theme?.bg;
	const transparentBg = bgSetting === 'transparent';
	const bg = transparentBg ? [0, 0, 0] : parseHexColor(bgSetting, [0, 0, 0]);
	const bgAlpha = transparentBg ? 0 : 255;
	for (let i = 0; i < imgW * imgH; i++) {
		const o = i * 4;
		pixels[o] = bg[0]; pixels[o + 1] = bg[1]; pixels[o + 2] = bg[2]; pixels[o + 3] = bgAlpha;
	}
	...
	for (let y = 0; y < height; y++) {
		for (let x = 0; x < width; x++) {
			...
			if (ch === ' ') continue; // skip blank cells
			...
			if (blockMode) {
				// Block mode: fill entire cell rectangle with solid colour
				for (let py = y * CELL_H; py < (y + 1) * CELL_H; py++) {
					for (let px = x * CELL_W; px < (x + 1) * CELL_W; px++) {
						const o = (py * imgW + px) * 4;
						pixels[o] = cellColor[0]; pixels[o + 1] = cellColor[1]; pixels[o + 2] = cellColor[2]; pixels[o + 3] = 255;
					}
				}
			} else {
				// Glyph mode: render ASCII art density pattern with coloured foreground
...
export function createAsciiGifWriter({ width, height, fps, render, outputPath }) {
	...
	const finalize = async () => {
		...
		for (let i = 0; i < renderedFrames.length; i++) {
			...
			if (hasTransparentBg) {
				// Map transparent pixels (alpha=0) to TRANSPARENT_INDEX.
				// With dispose:2, each frame is self-contained — no inter-frame
				// dedup because the canvas is cleared between frames.
				for (let j = 0; j < index.length; j++) {
					if (pixels[j * 4 + 3] === 0) {
						index[j] = TRANSPARENT_INDEX;
					}
				}
			} else if (prevIndexBuffer) {
				// Opaque mode: inter-frame LZW deduplication — overwrite
				// unchanged pixels with transparency (dispose:1 layers over previous)
				for (let j = 0; j < index.length; j++) {
			...
			encoder.writeFrame(index, imgW, imgH, {
				...
				delay,
				// dispose:2 = restore to background (needed for real transparency)
				// dispose:1 = keep frame (for opaque inter-frame dedup)
