 ...
 */
export async function convert({ inputPath, outputWidth = 100, color = false, onFrame, startTime, endTime, meta, collectFrames = true, targetFps, tone, charMode = 'ascii', foreground = null }) {
  ...
  const asciiHeight = Math.max(1, Math.round(outputWidth * (info.height / info.width) * (CELL_W / CELL_H)));
      ...
      while (buffer.length >= frameByteLength) {
        ...
        processing = processing.then(async () => {
          const fgMask = (fg && fg.mode === 'ml')
            ...
            : null;
          const frame = processFrame(pixels, scaledW, scaledH, outputWidth, evenHeight, sampleFactor, color, charMode, fg, fgMask);
...
async function buildMlMask(pixels, srcW, srcH, outW, outH, fg) {
  ...
  const feeds = {};
  feeds[inputName] = new _ortModule.Tensor('float32', input, [1, 3, modelSize, modelSize]);
 ...
 */
function processFrame(pixels, scaledW, scaledH, outW, outH, sampleFactor, color, charMode, foreground, fgMask) {
  ...
  if (useForeground) {
    if (foreground.mode === 'motion') {
      ...
      if (!_bgModelColors) {
        ...
        // Frame 1 is all background by definition (no motion yet)
      }
