...
export async function probeVideo(inputPath) {
	...
	return new Promise((resolve, reject) => {
		const args = [
			'-v', 'error',
			'-select_streams', 'v:0',
			'-show_entries', 'stream=width,height,r_frame_rate,sample_aspect_ratio:format=duration',
			'-of', 'json',
			inputPath,
		];
 ...
 */
export async function convert({ inputPath, outputWidth = 100, outputHeight, color = false, onFrame, startTime, endTime, meta, collectFrames = true, targetFps, tone, charMode = 'ascii', foreground = null, crop = null, signal = null, outlineOnly = false }) {
			...
			while (buffer.length >= frameByteLength) {
				...
				processing = processing.then(async () => {
					...
					const fgMask = (fg && fg.mode === 'ml')
						...
						: null;
					const frame = processFrame(pixels, scaledW, scaledH, outputWidth, evenHeight, sampleFactor, color, charMode, foreground, fgMask, outlineOnly);
...
function selectEdgeChar(tl, tr, bl, br, outlineOnly = false) {
 ...
 */
function processFrame(pixels, scaledW, scaledH, outW, outH, sampleFactor, color, charMode, foreground, fgMask, outlineOnly = false) {
	...
	for (let y = 0; y < outH; y++) {
		for (let x = 0; x < outW; x++) {
			...
			if (useBlocks) {
				...
				ch = selectEdgeChar(qTL / qDenom, qTR / qDenom, qBL / qDenom, qBR / qDenom, outlineOnly);
			}
