...
function buildColorDict(frames, qStep, palette) {
	console.log('[bundler] buildColorDict - palette:', palette ? `${palette.length} colors` : 'null', 'qStep:', qStep);
	const map = new Map();
	const dict = [];
	...
	function snap(c) {
		if (palette && palette.length) {
			const snapped = nearestPaletteColor(c, palette);
			return snapped;
		}
		return [
			quantizeChannel(c[0], qStep),
 ...
 */
function colorIndicesClose(idxA, idxB, dict, threshold) {
...
function deltaEncodeFrames(frames, color, dictInfo, threshold) {
	const encoded = [];
	let prevChars = null;
		...
		if (prevChars === null) {
			...
			for (let i = 0; i < curChars.length; i++) {
				...
				const colorChanged = color && curColorIndices && prevColorIndices &&
					!colorIndicesClose(curColorIndices[i], prevColorIndices[i], dict, threshold);
 ...
 *  - Duplicate:    { type: 0 }
 */
function deltaEncodeFramesBinary(frames, color, dictInfo, threshold) {
	const encoded = [];
	let prevChars = null;
	let prevColorIndices = null;
	const dict = dictInfo?.dict || null;
 ...
 */
export async function generateBundle({ frames, width, height, fps, color, outputDir, render, qStep }) {
	if (outputDir && isNode) {
	...
	const qStepVal = palette ? 1 : (typeof qStep === 'number' ? qStep : 24);
	const deltaThreshold = qStepVal * qStepVal * 1.05;

	// Build colour dictionary
	let dictInfo = null;
	let dictArray = null;
	if (color) {
		dictInfo = buildColorDict(frames, qStepVal, palette);
		dictArray = dictInfo.dict;
	}

	// V5 compact binary encoding
	const encodedFrames = deltaEncodeFramesBinary(frames, color, dictInfo, deltaThreshold);
	...
	if (isNode) {
		...
		compressed = gzipSync(rawBin, { level: 9 });
	} else {
		const stream = new Blob([rawBin]).stream().pipeThrough(new CompressionStream('gzip'));
	...
	if (outputDir && isNode) {
		...
		bundlePath = join(outputDir, 'bundle.js');
		htmlPath = join(outputDir, 'demo.html');
		await Promise.all([
 ...
 */
export function createBundleWriter({ width, height, fps, color, outputDir, render, qStep }) {
	const renderConfig = render || { mode: 'truecolor', theme: { fg: '#00ff00', bg: '#000000' } };
	const palette = renderConfig.mode === 'palette' ? renderConfig.palette : null;
	console.log('[bundler] createBundleWriter - renderConfig:', { mode: renderConfig.mode, paletteLength: palette ? palette.length : null });
	if (palette && palette.length > 0) {
		console.log('[bundler] Palette colors (first 3):', palette.slice(0, 3));
	}
	const qStepVal = palette ? 1 : (typeof qStep === 'number' ? qStep : 24);
	const deltaThreshold = qStepVal * qStepVal * 1.05;
	...
	const finalize = async () => {
		...
		let dictArray = null;
		if (includeColor) {
			dictInfo = buildColorDict(allFrames, qStepVal, palette);
			dictArray = dictInfo.dict;
		}

		// V5 compact binary encoding
		const encodedFrames = deltaEncodeFramesBinary(allFrames, includeColor, dictInfo, deltaThreshold);
		...
		const { bundleJS, bundleSize } = buildBundleJS(compressed);
		const demoHTML = buildDemoHTML(width, height, allFrames.length, fps);
		...
		if (outputDir && isNode) {
			...
			await writeFile(htmlPath, demoHTML, 'utf-8');
		}
...
function buildBundleJS(compressedBuf) {
	...
	if (typeof Buffer !== 'undefined') {
		b64 = Buffer.from(compressedBuf).toString('base64');
	} else {
		// Browser fallback for Uint8Array to base64
	...
	const js = [
		'// ASCII-fi bundle v5 – purely binary + gzip (native decompress)',
	...
	let bundleSize = 0;
	if (typeof Buffer !== 'undefined') {
		bundleSize = Buffer.byteLength(js, 'utf-8');
	} else {
		bundleSize = new Blob([js]).size;
	}

	return { bundleJS: js, bundleSize };
}
