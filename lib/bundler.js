...
// ────────────────────────────────────────────────────────────────────────────
// V4 compact encoding helpers
// ────────────────────────────────────────────────────────────────────────────

/**
 * Encode an array of unsigned integers as varint (LEB128) → base64.
 * Much more compact than JSON integer arrays, especially after gzip.
 */
function packVarints(arr) {
  const bytes = [];
...
/**
 * Hex-encode colour dictionary: [[r,g,b],...] → "RRGGBBRRGGBB..."
 * ~50% smaller than nested JSON arrays for colour data.
 */
function packColorDict(dict) {
  let hex = '';
  for (const c of dict) {
...
/**
 * Gap-encode positions (delta-of-deltas) then pack as varint base64.
 * Turns [3, 5, 8, 100] into gaps [3, 2, 3, 92] — much smaller varints.
 */
function packGaps(positions) {
  ...
  return packVarints(gaps);
}

/**
 * V4 char RLE: returns { cn: varint_b64_counts, cc: chars_string }
 * Instead of mixed-type array [3,"A",2,"B"], splits counts from chars
 * so integers and characters each compress better under gzip.
 */
function rleEncodeCharsV4(str) {
  if (!str.length) return { cn: '', cc: '' };
  const counts = [];
  let chars = '';
  let current = str[0];
  let count = 1;
  ...
  counts.push(count);
  chars += current;
  return { cn: packVarints(counts), cc: chars };
}

/**
 * V4 integer RLE → varint base64.  Interleaved [count, value, ...].
 */
function rleEncodeFromArrayV4(arr) {
  if (!arr.length) return '';
  const rle = [];
  ...
  rle.push(count, prev);
  return packVarints(rle);
}

/**
 * V4 delta encoding – same logic as V2 but uses compact packed formats:
 *  - Full frames:  { cn, cc, ci? }
 *  - Delta frames: { dp, dc, cp?, cv? }
 *  - Duplicate:    { d: 1 }
 */
function deltaEncodeFramesV4(frames, color, dictInfo) {
  const encoded = [];
  let prevChars = null;
  let prevColorIndices = null;
  const dict = dictInfo?.dict || null;
  ...
  for (const frame of frames) {
    ...
    if (prevChars === null) {
      // First frame – full data
      const rle = rleEncodeCharsV4(curChars);
      const enc = { cn: rle.cn, cc: rle.cc };
      if (color && curColorIndices) {
        enc.ci = rleEncodeFromArrayV4(curColorIndices);
      }
      encoded.push(enc);
    } else {
      ...
      if (!hasDiff) {
        ...
        if (ratio > 0.7) {
          ...
          const enc = { dp: packGaps(charPositions), dc: charValues };
          if (color && colorPositions.length) {
            enc.cp = packGaps(colorPositions);
            enc.cv = packVarints(colorValues);
          }
          encoded.push(enc);
        }
      }
...
export async function generateBundle({ frames, width, height, fps, color, outputDir, render }) {
  ...
  // V4 compact delta + RLE encode
  const encodedFrames = deltaEncodeFramesV4(frames, color, dictInfo);
  ...
  if (dictArray && dictArray.length) payload.cd = packColorDict(dictArray);
...
export function createBundleWriter({ width, height, fps, color, outputDir, render }) {
  ...
  const finalize = async () => {
    ...
    // V4 compact delta + RLE encode
    const encodedFrames = deltaEncodeFramesV4(allFrames, includeColor, dictInfo);
    ...
    if (dictArray && dictArray.length) {
      payload.cd = packColorDict(dictArray);
    }
...
function buildBundleJS(compressedBuf) {
  ...
  const js = [
    '// ascii-fy bundle v4 – gzip + varint + hex dict + gap deltas (native decompress)',
